...
def submit_batch_job(*, message_json: str, batch_queue: str) -> str:
    ...
    Reads the job queue and job definition from LAMBDA_TASKS_BATCH_QUEUES
    ...
    batch_queues = conf.BATCH_QUEUES

    if batch_queue not in batch_queues:
        raise ImproperlyConfigured(
            f"Batch queue '{batch_queue}' is not defined in LAMBDA_TASKS_BATCH_QUEUES."
        )

    queue_config = batch_queues[batch_queue]
    ...
    response = client.submit_job(
        jobName=job_name,
        jobQueue=queue_config["job_queue"],
        jobDefinition=queue_config["job_definition"],
        containerOverrides={
            "environment": [
